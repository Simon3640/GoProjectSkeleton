...
func GenerateFunction(fn FunctionConfig) error {
	// Calculate module path (e.g., "status/health_check" -> "goprojectskeleton/functions/aws/status/health_check")
	modulePath := fmt.Sprintf("goprojectskeleton/functions/aws/%s", strings.ReplaceAll(fn.Path, "/", "/"))
