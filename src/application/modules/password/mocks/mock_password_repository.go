...
type MockPasswordRepository struct {
	repositoriesmocks.MockRepositoryBase[dtos.PasswordCreate, dtos.PasswordUpdate, passwordmodels.Password, passwordmodels.PasswordInDB]
}
...
func (m *MockPasswordRepository) GetActivePassword(userEmail string) (*passwordmodels.Password, *applicationerrors.ApplicationError) {
	args := m.Called(userEmail)
	errorArg := args.Get(1)
	if errorArg != nil {
		return nil, errorArg.(*applicationerrors.ApplicationError)
	}
	return args.Get(0).(*passwordmodels.Password), nil
}
